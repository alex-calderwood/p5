...
function draw() {
   ...
   background(0,0,0,backgroundAlpha);
    ...
    var distX = random(dMin, dMax);
    var distY = random(dMin, dMax);
    if(frameCount % mod == 0) {
        let moddedDistX = (distX * frameCount) % windowWidth;
        let moddedDistY = (distY * frameCount) % windowWidth;
        point = start.copy().add(moddedDistX, moddedDistY);
        drawTick(point, g1);
    }
