...
for (let i = 1; i <= 3; i++) {
    ...
    corpusTextArea.value = Object.values(library)[i - 1];
}
...
// let channels = [1,2];
let channels = [2]; // The channel number must be in this list to be displayed
...
class Realization {
    ...
    edit(newValue) {
        this.realization = newValue.split(" ");
    }
}
...
class SlidyWindow {
    ...
    draw() {
        ...
        // draw outline around selected track
...
class Track {
    ...
    draw() {
        fill(128+(this.i*10));
        let line = textArray[this.i].toUpperCase();
        text(line, xLocs[this.i], this.i * this.trackHeight);
...
//     console.log("corpus changed", this.value());
//     grammar = library[this.value()].split(" ");
// }

function setup() {
    noStroke();
    var canvas = createCanvas(windowWidth, 1000);
...
function handleMIDIMessage(message) {
    ...
    if (eventType === 9) { // Note on message
        ...
        if (note === 1) { // Modulation wheel
            ...
            if (note === 70) {
                setSpeeds(velocity / 127 * maxSpeed);
            }
            else if (note === 74) {
